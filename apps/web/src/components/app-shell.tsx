...
export function AppShell({ screen, title, description, actions, children }: { screen: string; title: string; description?: string; actions?: React.ReactNode; children: React.ReactNode }) {
  ...
  const newInterviewHref = user ? "/setup" : "/auth/sign-up?next=%2Fsetup";
  ...
  return (
    <div className="min-h-[100dvh] bg-background">
      ...
      <aside className="fixed inset-y-0 start-0 z-30 hidden w-60 border-e bg-card/60 p-3 backdrop-blur md:flex md:flex-col">
        <div className="px-2 py-2"><Brand href={user ? "/dashboard" : "/"} /></div>
        <Button asChild className="mt-4 w-full"><Link href={newInterviewHref}><Plus aria-hidden="true" />New interview</Link></Button>
        ...
        <div className="space-y-1">
          ...
          {user ? <div className="flex items-center gap-2 rounded-md p-2">
            ...
            <span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{displayName}</span><span className="block truncate text-xs text-muted-foreground">{user?.email || "Candidate workspace"}</span></span>
      ...
      {menuOpen ? (
        <dialog ref={menuDialogRef} className="fixed inset-0 m-0 h-full max-h-none w-full max-w-none overscroll-contain border-0 bg-background/80 p-0 backdrop-blur-sm backdrop:bg-transparent md:hidden" aria-label="Navigation menu" onCancel={(event) => { event.preventDefault(); setMenuOpen(false); }}>
          <div className="h-full w-[min(19rem,86vw)] border-e bg-card p-4 pb-[max(1rem,env(safe-area-inset-bottom))] ps-[max(1rem,env(safe-area-inset-left))] pt-[max(1rem,env(safe-area-inset-top))] shadow-2xl">
            <div className="flex items-center justify-between"><Brand href={user ? "/dashboard" : "/"} /><Button ref={closeMenuRef} variant="ghost" size="icon" onClick={() => setMenuOpen(false)} aria-label="Close navigation"><X aria-hidden="true" /></Button></div>
            <Button asChild className="mt-6 w-full"><Link href={newInterviewHref} onClick={() => setMenuOpen(false)}><Plus aria-hidden="true" />New interview</Link></Button>
      ...
      <nav className="fixed z-30 grid grid-cols-5 rounded-xl border bg-card/95 p-1 shadow-xl backdrop-blur md:hidden" style={{ insetInlineStart: "max(0.75rem, env(safe-area-inset-left))", insetInlineEnd: "max(0.75rem, env(safe-area-inset-right))", bottom: "max(0.75rem, env(safe-area-inset-bottom))" }} aria-label="Mobile workspace">
